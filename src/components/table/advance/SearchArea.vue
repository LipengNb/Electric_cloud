  ...
  <div ref="root" class="search-area">
    ...
    <div v-for="(col, index) in searchCols" :key="index" class="search-item">
      ...
      <div v-else-if="col.dataType === 'date'" :class="['title', {active: col.search.value}]">
        ...
        <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title" />
        <a-date-picker v-model="col.search.value" :format="col.search.format" class="date-picker" size="small" :get-calendar-container="() => $refs.root" @change="onDateChange(col)" />
      </div>
      <div v-else-if="col.dataType === 'datetime'" class="title datetime active">
        ...
        <slot v-else-if="col.slots && col.slots.title" :name="col.slots.title" />
        <a-date-picker v-model="col.search.value" :format="col.search.format" class="datetime-picker" size="small" show-time :get-calendar-container="() => $refs.root" @change="(date, dateStr) => onCalendarChange(date, dateStr, col)" @openChange="open => onCalendarOpenChange(open, col)" />
      </div>
